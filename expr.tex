...
\begin{table*}[!t]
	...
	\begin{tabular}{c|c|c|c|c|c|c|c}
		...
		\hline\hline
		\textbf{Algorithm} & \multirow{2}{*}\textbf{g} & \textbf{f} & \textbf{Monot} &\textbf{Commu} & \textbf{Accumu} & \textbf{OrderInd} &\textbf{Conver}\\
		\hline
...
In this section,we test more algorithms to check the asynchronous conditions.We list $f$ and $g$ operations and each property of each algorithm .As shown in table \ref{tab:resut},\textbf{SSSP},\textbf{CC},\textbf{DAG},\textbf{PATH},\text{COST},\text{PARTY} and \textbf{Galaxy evolution} can be correctly asynchonized.\textbf{Belief Propagation},\textbf{Simrank},\textbf{Pagerank}and \textbf{Jacobi Method}can be covert to accumulative recursive aggregation model and asynchronous executed correctly. Note that 
