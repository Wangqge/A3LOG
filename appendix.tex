...
The formula as line(8) shown has the same result with the synchronous one.And it is obvious that the result can be extended to general situation.
 ...
 \begin{proof}
 	...
 	Since $g$ and $f$ satisfied the previous properties,the convertible condition of $G$ can be express as $G\circ F(X)=G'(X\cup G'\circ F(\Delta X))$ and we have $\Delta X^0=X^1=G\circ F(X^0)$\\
 	So:
 \begin{align}
&G'(\Delta X^0 \cup G'\circ F(\Delta X^0)\cup \ldots (G' \circ F)^{k-1}(\Delta X^0 )) \notag\\
=&G'(X^1 \cup G'\circ F(X^1 )\cup \ldots (G' \circ F)^{k-1}(X^1 )) \notag\\
=&G'(G'(X^1\cup G'\circ F(X^1 ))\cup \ldots (G' \circ F)^{k-1}(X^1 )) \tag{3}\\
=&G'(G\circ F(X^1)\cup \ldots (G' \circ F)^{k-1}(X^1)) \tag{4}\\
=&\ldots \notag\\
=&G'((G\circ F)^{k-2}(X^1)\cup(G' \circ F)^{k-1}(X^1)) \notag\\
=&G\circ F(X^{k-1})=X^k\notag
 \end{align}
Line 3 is true because of the accumulative property.Line 4 is true because the convertibility.As previous description, this is the Asynchronous Recursive Program Formula which has the same result with normal recursive formula.
