...
\emph{Recursive aggregation} is a sequence of interleaving aggregate and non-aggregate operations, which can be represented as follow.
\begin{equation}
\label{eq:recursive2}
\begin{aligned}
Y^{k}=F(X^k),\\
...
\subsection{Naive/Seminaive-Evaluation}
If evaluated with naive technology, The recursive rules will be repeatedly evaluating on the whole datasset untill terminate conditions are satisfied. Which incurs a lot of unnecessary computation. studies\cite{Seo:2013:DSD:2556549.2556572,Wang:2015:AFR:2824032.2824052,Lam:2013:SDE:2510649.2511289} showes that evaluating the origin recursive aggregation with naive-evaluation technology is very inefficient in datalog system. {\color{red}So seminaive-evaluation technology has been proposed\cite{Lam:2013:SDE:2510649.2511289},  the seminaive form is shown in equation \ref{eq:seminaive}, in which $-$ is set subtraction. This implies that only non convergence result will involved in the next step evaluation. 
	\begin{equation}
	\label{eq:naive}
	Naive:	R=(G\circ F)^k(X)\notag
	\end{equation}
\begin{equation}
\label{eq:seminaive}
\begin{aligned}
