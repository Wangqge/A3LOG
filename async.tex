...
{\color{red}
The first requirement is that the aggregation results can be incrementally computed as $ G^+\circ F(X^{k})=G^+(X^{k-1}\cup \Delta^{k-1})$. In which $G^+$ is the aggregate function. And as defined in Equation (\ref{eq:accumasync}), the increment $\Delta^{k}$ should be computed based on $\Delta^{k-1}$ using the same aggregate functions, i.e., $\Delta^{k+1}=G^+\circ F'(\Delta X^{k})$ where $F'$ is a new non-aggregate function. 
Then the Origin form $G^+\circ F(X^k)$ can be converted to 
\begin{equation}
\label{eq:convertform}
\begin{aligned}
&(G^+\circ F)^k(X)\\=&G^+(X^0\cup G^+\circ F'(\Delta^0)\ldots \cup (G^+\circ F')^{k-1}(\Delta^0))
\end{aligned}
\end{equation}
Since $G^+\circ F(X^{k-1})=G^+(X^{k-1}\cup\Delta^{k-1})$,  we have 
 $G^+\circ F(X^{k-1})=G^+(F(X^{k-2})\cup F'(\Delta^{k-2}))$ if $G^+$ has the accumulative property. Further, we have
\begin{equation}
\label{eq:findf}
\begin{aligned}
&G^+\circ (F(X^{k-1})-F(X^{k-2}))=G\circ F'(\Delta^{k-2}).
\end{aligned}
\end{equation}
}
...
\begin{definition}
	\label{th:convert}
	 A convertiable recursive program can be converted to an accumulated recursive program and return the same results after $n$ iterations with naive-evaluation, as long as the following conditions are satisfied:
	\begin{itemize}
		\item The aggregate operation $G^+$ is accumulative
%		\item The original program can be incrementaly evaluated with $ G^+\circ F(X^{k})=G^+(X^{k-1}\cup \Delta^{k-1})$
\item There exists an non-aggregate operation $F'$ satisfitd that $G^+\circ (F(X^{k-1})-F(X^{k-2}))=G^+\circ F'(\Delta^{k-2}).$
		\item The initial $\Delta^0$ is set with $X^1$-X^{0} with seminaive evaluation
