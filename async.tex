...
Furthermore, SMT solver Z3 can be used to find a qualified aggregate function $g'()$.
Following Corollary \ref{coro:auto:1}, we first use Z3 to check the satisfiability.
If the formula is not satisfiable, System will try to find a new $g'$ by the method proposed in sec 3.4.  If there is an $g'$ exist and satisfied the accumulative and order-independent condition,SMT solver will return $g'$.  Otherwise it will return unknown or unsatisfiable. 
