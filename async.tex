...
\begin{definition}
	\label{def:asyncaggre}
	(\textbf{Asynchronous Aggregation}) In a recursive program,for each key $k_p$ we assume that the set $Y_{k_p}^k$ from any $k$th recursion is partitioned into $m$ disjoint subsets, i.e., $Y_{k_p}^k=\{Y_{k_p 1}^k,Y_{k_p2}^k,\ldots,Y_{k_pm}^k\}$, and $\forall i,j, Y_{k_pi}^k\cap Y_{k_pj}^k=\emptyset$. Asynchronous aggregation is to aggregate multiple subsets from different recursions, i.e., $g(\ldots Y_{k_pi}^k\ldots\cup Y_{k_pj}^{l}\ldots)$ where $k\neq l$.
	
	Similarly, for a group-by aggregate operation. the whole set can be partitioned into $m$ disjoint subsets,i.e. $Y^k=\{Y_{ 1}^k,Y_{2}^k,\ldots,Y_{m}^k\}$  and $\forall i,j, Y_{i}^k\cap Y_{j}^k=\emptyset$. Asynchronous group by aggregation is to aggregate multiple subsets from different recursions, i.e., $G(\ldots Y_{i}^k\ldots\cup Y_{j}^{l}\ldots)$ where $k\neq l$.
