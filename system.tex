...
\Paragraph{Message passing.}Similar to Grape$+_{AP}$,  share-nothing enginee adopt a point to point communication and push-based  message passing: local worker $W_i$sends message $M(i,j)$ to worker$W_j$ as soon as the size of $M(i,j)$ reach the threshold $\beta{(i,j)}$.
