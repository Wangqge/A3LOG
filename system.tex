...
There are two mainstream strategy for asynchronous processing. Block-centric  T
Neither vertex-centric processing and block-centric can effectively reduce the stale computation. The state-of-art approach proposed to using a hybrid mode based on block-centric model to dynamically adjusts the message sending delay to achieve BSP (sync.), AP (async.), and SSP (between sync. and async.). The 
