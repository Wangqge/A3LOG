...
In order to translate $g$ and $f$ into a formula for being verified by an SMT solver, we implement a serie of unary function $H_{fi}(*)$  to represent the $i$th output expression of $f$, and Binary function $H_g(*,*)$ to represent aggregate operation $g$. If the accumulative property is satisfied
We can further define $H^n_g()$ for $n$ inputs aggregation by recursively apply $H_g(*,*)$.
