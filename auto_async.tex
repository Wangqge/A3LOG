...
The nonaggregate operation is constract with relation and output expression, since there might be several output in one rules e.g., the example 2, in \texttt{r2.}, the $F$ is divided into two parts,
\begin{lstlisting}
1. assb(Part,Sub,$n$), cost(Sub,$c$), $\mathcal{C}=c*n$;
2. basic(Part,$\mathcal{C}$).
\end{lstlisting}
and the output expression of non aggregate operation is also consist of the part, $\mathcal{C}=c*n$ and  $\mathcal{C}$.


First, the $f()$ and $g()$ functions and the conditions have to be translated into a formula for being verified by an SMT solver. So we implement a serie of unary function $H_{fi}(*)$  to represent the $i$th output expression of $F$, and Binary function $H_g(*,*)$ to represent aggregate operation $G$. If the accumulative property is satisfied
We can further define $H^n_g(*)$ for $n$ inputs aggregation base on the accumulative property.
...
\begin{definition}
	...
	Denote $H_f(*)$ as set of the output expression, e.g., $H_f(*)=\{H_{f0}(*),H_{f1}(*)\ldots H_{fn}(*)\}$. And
	...
	\begin{itemize}
		...
		$H_l=H^*_g(H_f(H^*_g(x_1,x_2),H^*_g(x_1,x_2)),e)$\\		
		 and  $H_r=H^*_g(H_f(H^*_g(x_1,x_2)),H_f(H^*_g(x_1,x_2)))$;
	\end{itemize}
\end{definition}
{\color{green}
