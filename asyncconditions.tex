...
{
Previous studies\cite{Seo:2013:DSD:2556549.2556572,Wang:2015:AFR:2824032.2824052,Lam:2013:SDE:2510649.2511289} showes that evaluating the origin recursive aggregation with naive-evaluation technology is very inefficient in datalog system . The program will terminate only if all the aggregate result converged to their fixpoints. {\color{red}So seminaive-evaluation technology has been proposed\cite{Lam:2013:SDE:2510649.2511289},

Socialite\cite{} proposed that a small class of aggregates, $meet$ aggregated, can be evaluated recursively, if these aggregate operations are associative, commutative, and idempoent, such as $\$min$ and $\$max$.Myria \cite{} further proposed $bag-monotonic$ aggregates conditions that removed the idempoent constraint and expand the application scope to more popular aggregate functions including $\$min$, $\$max$, $\$sum$ and $\$count$.
These work shows that if aggregate operator is $bag-monotonic$ and non-aggregate operation $F$ is monotonic, the recursive program can be semi-naive evaluating. 		

Comparing with naive evaluation technology,semi-naive technology can puring most redudent computation. To utilize both semi-naive technology and asynchronous aggregation , we further proposed \textbf{accumulative recursive aggregation} by improving semi-naive technology. First we give the definition.}
...
\begin{definition}
	...
	G\Big(\Delta X^0\cup \Delta X^q\cup\ldots\cup (\Delta X^k)\Big).
	\end{equation}
\end{definition}
	
accumulative recursive aggregation program can be evaluated with semi-naive technology,
...
Based on the previous fundation, The accumulative further require the difference $\Delta X$ can be iteratively computed as equation \cite{\label{eq:accumasync}} 

So based on these observation we summarize the correctness conditions of accumulated recursive
}
...
\begin{definition}
	...
	\begin{itemize}
        ...
        \item \textbf{UNDEFINE-NAME}: $F(X_1)-F(X_2)=F(X_1-X_2)$
...
Since UNDEFINE-NAME property, we have
\begin{align}
\Delta X^{k}&=X^{k+1}-X^{k}\\\notag
&=G\circ F(X^k)-G\circ F(X^k-1)\\\notag
&=G(F(X^k)-F(X^{k-1}))\\\notag
&=G\circ F(X^k-X^{k-1})\\\notag
