...
aggregation means a data fusion operation which frequently used in Graph computing  machine learning and database query.Many recursive algorithm can be abstracted as a series of aggregation and non-aggregation operations(recursive aggregation).Asynchronous aggregation was proposed to accelerate the aggregation the and avoid the coordination.But the non-guaranteed correctness limit the usage. moreover it is hard to write asynchronous program or check whether an algorithm can be correctly asynchronized.To address there problem we propose 1) a series of correctness conditions based on \textbf{accumulate recursive aggregation}.2)a Datalog System with asynchronous condition checker embedded.User's sequentials program will be checked and convert to accumulate recursive aggregation and async-executed,if the correctness condition satisfied.System provide both shared-memory and distributed runtime engine,experiment shows that our algorithm shows comparable and most time even better performance.
\end{abstract}
